...
パスのうちローカルの環境に依存する部分だけパース中に解決する案。(ローカルの環境に依存しない部分、例えば `型名::メソッド名` みたいなのパース中に解決するのは無理なので、そのまま持つ。)
...
- ノードがパースされるたび、定義位置に含まれる名前を環境に入れる。
- スコープから抜けるたび、そのスコープに閉じている名前を環境から取り除く。
- 相対のパス式や単独で出現する名前がパースされたとき、先頭の名前 (ローカル環境に依存する) を解決する。解決できなければ名前をマップに入れる。(このタイミングで解決できないということは、前方参照だと思っててよい。)
- hoisting の対象となる宣言がシンボルを導入したとき、マップからその名前を探し、元となる識別子 (パス式の先頭部分) をこのシンボルに解決する。解決した名前はマップから取り除く。
